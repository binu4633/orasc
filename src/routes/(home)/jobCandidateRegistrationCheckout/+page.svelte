<script lang="ts">
  ...
  if(data?.fees?.fees?.jobCandidateRegistrationFee){
    registrationFee = data.fees.fees.jobCandidateRegistrationFee
  ...
  async function handlePayment() {
   ...
   const response = await fetch("/api/jobCandidateRegistration", {
     method: "POST",
     headers: {
  ...
  <div class="w-full min-h-screen bg-primary text-center pt-10">
    ...
    <div class="w-full flex justify-center">
      <p class=" max-w-fit text-3xl px-5 border-b-2 border-black my-5">summary</p>
    </div>
