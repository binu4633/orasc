...
export const POST: RequestHandler = async ({request,locals}) => {
    ...
    if(!user.jobCandidateRegistrationId){
      return new Response(JSON.stringify({ error: 'please register candidate form' }), { status: 500 });
    }

    const feesDb = await Fees.find();

    const fees = feesDb[0].jobCandidateRegistrationFee ||0
    Number(fees)
    const options = {
