<script lang="ts">
   ...
   async function updateHandler(){
      if(data?.university?._id && approveSelected){
    const response = await fetch(`/api/approve/university`,{
      method:"PATCH",
      body: JSON.stringify({
        id:data.university._id,
  ...
  <section class="bg-primary min-h-screen">
      <div class="flex justify-center">
        ...
        <option value="unapproved">unapproved</option>
          ...
          <button onclick={updateHandler} class=" bg-primary-compliment px-4 md:btn-second">update</button>
      </div>
      ...
      {#if data?.university}
        <table>
          ...
          <tbody>
            ...
            {#each Object.entries(data.university) as [key, value], index (key)}
