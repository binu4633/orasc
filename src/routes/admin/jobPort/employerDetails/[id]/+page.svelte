<script lang="ts">
   ...
   async function updateHandler(){
      if(data?.employer?._id && approveSelected){
    const response = await fetch(`/api/approve/employer`,{
      method:"PATCH",
      body: JSON.stringify({
        id:data.employer._id,
      ...
      {#if data?.employer}
        <table>
          ...
          <tbody>
            ...
            {#each Object.entries(data.employer) as [key, value], index (key)}
