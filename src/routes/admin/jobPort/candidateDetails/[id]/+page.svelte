<script lang="ts">
  import { goto, invalidate } from "$app/navigation";
  import { page } from "$app/state";
  import type { PageData } from "./$types";

  let { data }: { data: PageData } = $props();
  let approveSelected = $state();

  // console.log('data',data.candidate.cv)

  async function updateHandler() {
    console.log("before");
    
    if (data?.candidate?._id && approveSelected) {
      console.log("after");
      console.log(
        "update handler worksss",
        data.candidate._id,
        approveSelected
      );

      const response = await fetch(`/api/approve/candidate`, {
        method: "PATCH",
        body: JSON.stringify({
          id: data.candidate._id,
          approve: approveSelected,
        }),
      });
      const result = await response.json();
      console.log("result", result);
      if (result?.status === "success") {
        invalidate(page.url.href);
      }
    }
  }

  function fileDownload(f) {
    const data = Uint8Array.from(f.data.data);

    const blob = new Blob([data.buffer], {
      type: "application/pdf",
    });

    const url = window.URL.createObjectURL(blob);
    console.log("url", url);
    window.open(url, "_blank");
    window.URL.revokeObjectURL(url);
  }
</script>

<section class="bg-primary min-h-screen">
  <div class="flex justify-center">
    <select
      name="approve"
      id="approve"
      class="border-0 bg-primary text-xl text-white-400 mx-4"
      bind:value={approveSelected}
    >
      <option value="">select one</option>
      <option value="pending">pending</option>
      <option value="approved">approved</option>
      <option value="unapproved">unapproved</option>
    </select>
    <button onclick={updateHandler} class="btn-second">update</button>
  </div>
  <div class="center-col pb-10">
    {#if data?.candidate}
      <table>
        <tbody>
          {#each Object.entries(data.candidate) as [key, value], index (key)}
            <tr class="m-10 py-3 md:text-2xl">
              <td class=" py-3 px-2">{key}:</td>
              <td>{value}</td>
            </tr>
          {/each}
        </tbody>
      </table>
      {#if data?.candidate?.cv}
        <button
          onclick={fileDownload.bind(null, data.candidate.cv)}
          class="btn-second">download cv</button
        >
      {/if}
    {:else}
      <p>sorry cannot find result</p>
    {/if}
  </div>
</section>
