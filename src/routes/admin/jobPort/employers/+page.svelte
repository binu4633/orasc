<script lang="ts">
    ...
    $effect(() => {
     if(data?.employer?.totalPages){
     
     const arr = [];
     for(let i =1;  i<=data.employer.totalPages;i++){
    ...
    if(data?.employer?.page){
     pageSelected= data.employer.page
    }
      });
    ...
    function detailsHandler(id: any) {
      goto(`/admin/jobPort/employerDetails/${id}`);
    }
  ...
  <section class="w-full min-h-screen bg-primary">
    ...
    {#if data.employer?.employer && data.employer.employer.length > 0}
      {#each data.employer.employer as employer}
        <div class="w-full center-col  m-4 bg-primary-dark p-6 text-white-400">
          <p>buisness name:{employer.businessName}</p>
          <p>country:{employer.country}</p>
          <p>city:{employer.city}</p>
          <p>approve:{employer.registrationApprove}</p>
          <div>
            <button
              onclick={detailsHandler.bind(null, employer._id)}
              class="btn-second"
              >details
            </button>
          </div>
