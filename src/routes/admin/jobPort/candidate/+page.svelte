<script lang="ts">
    ...
    $effect(() => {
     if(data?.candidate?.totalPages){
    
     const arr = [];
     for(let i =1;  i<=data.candidate.totalPages;i++){
    ...
    if(data?.candidate?.page){
     pageSelected= data.candidate.page
    }
      });
    ...
    function detailsHandler(id: any) {
      goto(`/admin/jobPort/candidateDetails/${id}`);
    }
  ...
  <section class="w-full min-h-screen bg-primary">
    ...
    {#if data.candidate?.candidate && data.candidate.candidate.length > 0}
      {#each data.candidate.candidate as candidate}
        <div class="w-full center-col  m-4 bg-primary-dark p-6 text-white-400">
          ...
          <p>Approve:{candidate.registrationApprove}</p>
          <div>
            <button
              onclick={detailsHandler.bind(null, candidate._id)}
              class="btn-second"
              >details
            </button>
          </div>
